...
이 repo 는 **독립 2-트리** 모델을 택했다 (`/on-call-elian` 리뷰에서 의식적으로 결정). **단일 진실원이 없다.** `codex/prompts/on-call-elian.md` 와 `plugins/elian-store/skills/on-call-elian/SKILL.md` 는 *별개 파일* 이고, 한쪽 로직을 바꾸면 다른 쪽 동기화는 **작성자 수동 책임**이다.

> 한쪽만 고치고 다른 쪽을 잊으면 두 도구의 동작이 갈린다. 이게 정확히 v2.5.0 에서 on-call-elian 안에서 잡았던 "산문↔절차 drift" 버그의 트리-레벨 버전이다. PR 시 두 파일 diff 를 같이 확인하라.

## 설치

```bash
# 1) 커스텀 프롬프트 (스킬 analog) — /on-call-elian 로 호출 가능해짐
...
설치 후 Codex TUI 에서 `/on-call-elian <target> [--depth quick|deep|interview]` 로 사용.
...
codex/
  ...
  prompts/
    on-call-elian.md   ← /on-call-elian 의 Codex 네이티브 포팅 (레퍼런스)
  config.toml.example  ← ~/.codex/config.toml 샘플 (read-only 리뷰 안전 기본값)
```

스킬 포팅 범위: 현재 `on-call-elian` 1개만 (레퍼런스). 나머지 elian-store 스킬은 패턴 검증 후 점진 추가.
