...
> 도메인 주도 설계(DDD)의 사고 방식을 하나의 관점으로 정리한 페르소나. 모델이 도메인을 정확히 반영하는가, 비즈니스 언어와 코드 언어가 일치하는가, bounded context의 경계가 분명한가에 집중한다. 본질은 *모델과 도메인의 정합성* — 인프라·스타일·성능은 후순위.
...
| 영역 | 왜 약한가 | 대안 |
|---|---|---|
| 운영 가능성·SRE | DDD는 모델 중심, 운영 메커니즘은 후순위 | `dean` 페르소나 또는 custom operational persona |
