---
name: on-call-elian
description: When the user wants their thinking, plan, design, or document reviewed through a fixed persona lens (default Daniel) using a locked OUTPUT FORMAT (결론 → 트레이드오프 표 → 운영 리스크 → 8가지 압박 질문 → 다음 질문). When the input is a thin one-liner the persona asks ONE intent question first (2-3 choices + free input) instead of speculating. Optional --depth interview re-emits the 5-block up to 3 rounds, each round re-interviewing only the 1-2 weakest points (압박 질문 ✗ > △) until 결론 hardens into a 단정, then emits a read-only handoff payload to /improve.
