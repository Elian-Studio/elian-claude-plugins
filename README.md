...
| [functional-spec](plugins/elian-store/skills/functional-spec/) | Bridge a wireframe to a code-grounded implementation contract: per-screen functional decomposition + component contract (real reuse paths, endpoints, new-vs-existing), plus a wireframe↔spec split "connected" HTML view. Sits after `/design-ui` mockups, before `/implement`. | `/elian-store:functional-spec` |
